...
class Pow(pygame.sprite.Sprite):
	def __init__(self, center, dx, dy):
		pygame.sprite.Sprite.__init__(self)
		self.type = random.choice(['gun', 'shield', 'reload'])
...
def show_game_begin_screen():
	...
	draw_text(screen, "Pause: escape", 18, WIDTH / 2, HEIGHT * 18 /32)
...
def show_game_over_screen():
	...
	draw_text(screen, "Score: {}".format(str(score)), 24, WIDTH / 2, HEIGHT * 7 / 16, TITLE_FONT)
	draw_text(screen, "Kills: {}".format(str(enemy_kill_count)), 24, WIDTH / 2, HEIGHT * 8 / 16, TITLE_FONT)
...
reload_powerup_img = pygame.image.load(path.join(img_dir, "star_silver.png")).convert()
# Enemies
enemy_images = []
...
powerup_images['reload'] = pygame.image.load(path.join(img_dir, 'star_silver.png'))
